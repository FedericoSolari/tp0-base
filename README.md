...
En esta etapa se incorporó un nuevo ciclo de comunicación entre cliente y servidor: la notificación explícita de finalización de apuestas y la posterior consulta de ganadores.

En el cliente, al terminar de enviar todos los batches, se envía un mensaje de cierre (AllBetsDone), indicando al servidor que no habrá más apuestas:
```Go
if err := handler.SendAll([]byte(AllBetsDone()))
```

Una vez confirmada la finalización, el cliente espera la señal de inicio del sorteo (BeginLottery) y procede a recibir, de forma secuencial, los documentos de los ganadores asociados a su agencia. Este intercambio está implementado en los métodos waitBeginLottery y receiveWinners:
```Go
...
Finalmente, tras obtener la lista completa, el cliente registra en el log la cantidad de ganadores recibidos:
```Go
log.Infof("action: consulta_ganadores | result: success | cant_ganadores: %d", len(winners))
```

En el servidor, la novedad principal es la sincronización entre todas las agencias. Cada vez que un cliente envía el mensaje AllBetsDone, se marca la sesión como finalizada. Cuando los cinco clientes han completado esta notificación, el servidor inicia el sorteo con el método beginLottery:
```Py
...
La lógica garantiza que no se consulta a los ganadores hasta que todas las agencias cierran su envío de apuestas, evitando resultados parciales. Además, los DNIs ganadores se envían únicamente a la agencia correspondiente, respetando el criterio de aislamiento de resultados.
A su vez, para relacionar el codigo de agencia con la conexion a un cliente, se implemento un diccionario de manera complementaria a la lista de conexiones. De esta forma, al recibir por primera vez un mensaje de una apuesta, se relaciona el codigo de agencia del emisor de la apuesta con la conexion establecida.
Por lo tanto, para informar los ganadores se opto por un diseño que solo recorre todas las apuestas realizadas una vez y al detectar un ganador se lo envia directamente a la agecia correspondiente, sin necesidad de que la agencia luego consulte los ganadores.

El ciclo de comunicación se completa con dos mensajes de control: NoMoreWinners, que marca el final de la lista, y EndMessage, que indica el cierre definitivo de la sesión.

---
### Ejercicio 8

En este punto se extendió el servidor para que pueda aceptar múltiples conexiones en paralelo y procesar los mensajes de cada cliente de manera concurrente. Para lograrlo, se utilizó el módulo threading de Python, donde cada cliente aceptado por el accept() del socket principal dispara la creación de un nuevo hilo dedicado a manejar su sesión completa.
De esta forma, el servidor puede continuar aceptando nuevos clientes, hasta cumplir con la cantidad de clientes esperados, mientras otros se encuentran enviando apuestas, sin bloquear el ciclo principal.

Para garantizar la consistencia de los datos compartidos, se incorporaron distintos mecanismos de sincronización:
* self._clients_lock: asegura el acceso seguro a la lista de conexiones activas.
* self._agency_lock: protege el registro que relaciona agencias con sus sockets.
* self._bets_lock: serializa la persistencia de apuestas en disco (utils.store_bets).
* self._done_lock y self._all_done: coordinan la señalización de que todos los clientes finalizaron.

Ejemplo de uso de Lock para persistir apuestas:
```Py
with self._bets_lock:
    utils.store_bets([bet])
```

Esto evita condiciones de carrera y garantiza la integridad de la persistencia incluso con múltiples hilos escribiendo en paralelo.

Luego de leer el informe proporcionado por la catedra acerca de la limitacion del multithreading en python, conclui que en este proyecto no representa una limitación significativa.
Debido a que, principalmente el servidor se encarga de acciones de entrada y salida unicamente, permitiendo que mientras se esperan datos otros hilos avancen. A su vez, el procesamiento de las apuestas no representa una accion cpu intensive.

Por ultimo la funcion beginLottery, esta desarrollada de manera threadsafe, utilizando locks en cada accion "critica", sin embargo no es necesario realizarlo asi ya que a esa funcion solo accede el hilo principal, luego de ya haber cerrado/joineado todos los hilos de los clientes.
Solo no se modifico el diseño de la solucion, por cuestiones de tiempo.
